...
def handler(event, ctx):
    ...
    if msg_type == "STATE":
        # host sends authoritative state; relay to both players
    ...
    if msg_type == "SCORE":
        ...
        for it in resp.get("Items", []):
            if it.get("matchId") == match_id:
                _post(
                    it["connectionId"],
                    {"type": "SCORE_UPDATE", "scoreA": scoreA, "scoreB": scoreB},
                )
    ...
    if msg_type == "PLAY_AGAIN":
        ...
        for it in resp.get("Items", []):
            if it.get("matchId") == match_id:
                _post(
                    it["connectionId"],
                    {"type": "REPLAY_STATUS", "agree": body.get("agree", False)},
                )
        return {"statusCode": 200, "body": "ok"}
