...
if (!HTTP) console.error("ENV.HTTP_API_BASE not set; check inline config block.");
if (!WS_URL) console.warn("ENV.WS_URL not set; WebSocket features will be disabled.");
...
$('guestBtn').onclick = async () => {
  try {
    if (!HTTP) { log("Guest: API base not configured."); return; }
    const name = $('username').value || undefined;
    const r = await fetch(`${HTTP}/auth/guest`, jsonReq('POST', { username: name }));
    const txt = await r.text();
    let j; try { j = JSON.parse(txt); } catch { j = { raw: txt }; }
    if (!r.ok) { log('Guest error: ' + JSON.stringify(j)); return; }
    token = j.token; userId = j.userId; username = j.username || j.userId;
...
$('signupBtn').onclick = async () => {
  try {
    ...
    const r = await fetch(`${HTTP}/auth/signup`, jsonReq('POST', { username: u, password: p }));
    const j = await r.json().catch(() => ({}));
    if (r.ok) {
      token = j.token; userId = j.userId; username = u;
...
$('loginBtn').onclick = async () => {
  try {
    ...
    const r = await fetch(`${HTTP}/auth/login`, jsonReq('POST', { username: u, password: p }));
    const j = await r.json().catch(() => ({}));
    if (r.ok) {
      token = j.token; userId = j.userId; username = u;
  ...
  try {
    ...
    if (lbTableBody) {
      ...
      (j.items || []).forEach(row => {
        ...
        tr.innerHTML = `<td>${row.username || row.userId}</td><td>${row.tier || ''}</td><td>${row.score || 0}</td>`;
...
async function pollForMatch() {
  try {
    const r = await fetch(`${HTTP}/matchmaking/check`, jsonReq('POST', { userId }));
    const j = await r.json().catch(() => ({}));
...
$('startBtn').onclick = async () => {
  try {
    ...
    const r = await fetch(`${HTTP}/matchmaking/start`, jsonReq('POST', { userId }));
    const j = await r.json().catch(() => ({}));
    log(`Start response: ${JSON.stringify(j)}`);

    if (r.ok && j.matchId) {
      // immediate match (second player)
      matchId = j.matchId; opponentId = j.opponent;
      setRole();
...
function startWebSocket() {
  if (!WS_URL) { log('WS_URL not configured.'); return; }
  ...
  ws.onmessage = (ev) => {
    try {
      ...
      if (msg.type === 'REPLAY_STATUS') log(`Replay: opponent agree=${msg.agree}`);
    } catch (e) {
...
function sendInput(v) {
  if (ws && ws.readyState === 1)
    ws.send(JSON.stringify({ type: 'INPUT', userId, matchId, axis: 'y', value: v, ts: Date.now() }));
}
...
function resetBall(dir) {
  ...
  ball.vx = 3 * dir; ball.vy = (Math.random() > 0.5 ? 2 : -2);
}
...
function tick() {
  ...
  if (role === 'host') {
    ...
    // collisions
    if (ball.x - ball.r < paddleA.x + paddleA.w &&
        ball.y > paddleA.y && ball.y < paddleA.y + paddleA.h) {
      ball.vx = Math.abs(ball.vx);
    }
    if (ball.x + ball.r > paddleB.x &&
        ball.y > paddleB.y && ball.y < paddleB.y + paddleB.h) {
      ball.vx = -Math.abs(ball.vx);
    }

    // scoring
    if (ball.x < -10) { scoreB++; notifyScore('B'); resetBall(-1); }
    if (ball.x > W + 10) { scoreA++; notifyScore('A'); resetBall(1); }
  } else {
...
function onGameOver(msg) {
  ...
  show('playAgainBtn', true); show('exitBtn', true);
}

$('playAgainBtn').onclick = () => {
  show('playAgainBtn', false); show('exitBtn', false);
  if (ws && ws.readyState === 1)
    ws.send(JSON.stringify({ type: 'PLAY_AGAIN', userId, matchId, agree: true }));
